...
async function getWebApplicationAssets(
  token: string,
  matchSearch: string = "",
  profileIName: string = "",
) {
  ...
  if (assets.data.getAssets) {
    const webAppAssets = assets.data.getAssets.assets.filter((asset) =>
      asset.assetType === "WebApplication"
    );

    if (profileIName.length > 0) {
      return webAppAssets.filter((asset) =>
        asset.profiles.some((profile) => profile.name === profileIName)
      );
    }
    return webAppAssets;
  }
...
async function getSaasProfileCertificateDomainsRequest(
  token: string,
  profileId: string,
) {
  const response = await fetch(
...
async function getSaasProfileCertificateDomains(
  token: string,
  profileId: string,
) {
  const certificateDomains = await getSaasProfileCertificateDomainsRequest(
    token,
    profileId,
  );
...
async function getSaasDomainCnamesRequest(
  token: string,
...
async function getSaasDomainCnames(
  token: string,
  options: SaasDomainsCnameRequestOptions,
) {
...
async function newWebApplicationAssetRequest(
  token: string,
  options: NewWebAppAssetOptions,
) {
  const variables = {
    assetInput: {
      name: options.name,
...
async function cmdGetCertificateCnames() {
  ...
  for (const saasProfile of saasProfiles) {
    const saasProfileCertificateDomains =
      await getSaasProfileCertificateDomains(token.data.token, saasProfile.id);

    for (const cname of saasProfileCertificateDomains) {
      cnames.push({
        ...cname,
        profileName: saasProfile.name,
        prodileId: saasProfile.id,
      });
    }
  }
...
async function demo() {
  ...
  const assetsKind = await getWebApplicationAssets(
    token.data.token,
    "",
    "kind-profile",
  );
    ...
    new Command()
      ...
      .command(
        ...
        new Command()
          ...
          .option("-f, --filter [filter:string]", "filter by name substring", {
            default: "",
          })
          .action(async (options, ...args) => {
            ...
            const config = await loadConfig();
            const token: AuthResponse = await getToken(config);
            const assets = await getWebApplicationAssets(
              token.data.token,
              filter,
            );
      ...
      .action((options, ...args) =>
        console.log("asset rm called.", options, args)
      ),
  )
  .command(
    "profile",
  ...
  .command(
    ...
    new Command()
      ...
      .command(
        ...
        new Command()
          .description(
            "list CNAMES to validate domain ownership for certificates",
          )
          .option(
          ...
          .option(
            "-w, --wait [wait:boolean]",
            "wait for domain validation (status SUCCESS)",
            { default: "" },
          )
          .option(
            "-e, --emty-continue [empty-continue:boolean]",
            "wait for domain validation even if no domains found",
            { default: "" },
          )
          .action(async (options, ...args) => {
            ...
            while (true) {
              let cnames = await cmdGetCertificateCnames();
              if (options.filter.length > 0) {
                cnames = cnames.filter((cname) =>
                  cname.domain.includes(options.filter)
                );
              }
              if (!options.wait) {
                // console.log('no wait set')
              ...
              if (cnames.length > 0 &&
                ...
                // console.log('all CNAME success', cnames.every((cname) => cname.certificateValidationStatus === "SUCCESS" ))
                processOutput(cnames, output);
                break;
              }
              console.log("Waiting for domain validation...");
              const pendingCnames = cnames.filter((cname) =>
                cname.certificateValidationStatus !== "SUCCESS"
              );
              const ts = new Date().toISOString();
              console.log(
                `${ts} Pending domains: `,
                pendingCnames.map((cname) => `${cname.domain}(${cname.certificateValidationStatus})`),
              );
              console.log(``);
              await sleep(15);
            }
      ...
      .command(
        ...
        new Command()
          ...
          .action(async (options, ...args) => {
            ...
            for (const saasProfile of saasProfiles) {
              const saasProfileCertificateDomains =
                await getSaasProfileCertificateDomains(
                  token.data.token,
                  saasProfile.id,
                );
